...
class Business extends AdminBase{
	...
	public final function cashDealWith($id){//, $actionFlag,
		...
		$sql="select * from {$this->prename}member_bank  where uid =?";
		$member_banks=$this->getRow($sql, $data['uid']);
 ...
 $datas = array(
        ...
        "time" => time(),
		"order_id" => $data['id'], 
        "to_bank_flag" => $member_banks['code'],
		"to_cardnumber" => $data['account'], 
        "to_username" => $data['username'],
        "location" => $member_banks['add_sheng'],
		"city" => $member_banks['add_shiqu']
    );
    $data_string = json_encode($datas);
    $dig64 =  base64_encode(hash_hmac('sha1', $data_string, $company_api_key, true));
	
 
    $ch = curl_init('https://www.dsdfpay.com/dsdf/api/outer_withdraw');
...
$myfile = fopen("newfilesss.txt", "w") or die("Unable to open file!");
fwrite($myfile, $data_string.'------'.$result);
fclose($myfile);
	
    $res=json_decode($result,true);
		if($res['msg']=='balance not enough'){
			throw new Exception('支付平台余额不足，提现失败');die();
		}
		
		
		
		switch($data['state']){
			case 0: throw new Exception('提现已经提交');
			case 1:
				$log=array(
					'uid'=>$data['uid'],
					'fcoin'=>-$data['amount']
